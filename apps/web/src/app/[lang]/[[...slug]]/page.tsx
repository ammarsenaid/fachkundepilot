        ...
        {isHome ? (
          <>
            <section className="grid gap-8 rounded-[2rem] border border-white/70 bg-white/55 p-6 shadow-[0_35px_90px_rgba(15,23,42,0.12)] backdrop-blur-xl md:grid-cols-2 md:gap-12 md:p-12">
              <div>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <Link href={buildLocalizedPath(lang, "register")} className="rounded-2xl bg-[var(--primary)] px-6 py-3 text-sm font-semibold text-white shadow-[0_14px_36px_rgba(11,58,117,0.35)] transition hover:-translate-y-0.5">Kostenlos starten</Link>
                  <Link href={buildLocalizedPath(lang, "materials")} className="rounded-2xl border border-slate-200 bg-white/95 px-6 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-white">Lernmaterial ansehen</Link>
                </div>
              </div>
              <aside className="rounded-3xl border border-white/80 bg-slate-950 p-5 text-white shadow-2xl md:p-6">
                <p className="text-xs uppercase tracking-[0.14em] text-slate-300">Lern-Dashboard</p>
                <h2 className="mt-3 text-2xl font-semibold">Dein Prüfungspfad auf einen Blick</h2>
            ...
            <section className="mt-8 grid gap-4 md:grid-cols-3">
              {["Mehrsprachig lernen", "Prüfungsorientierte Inhalte", "Schritt-für-Schritt Fortschritt"].map((benefit) => (
            ...
            <section className="mt-12 grid gap-6 md:grid-cols-2">
              <article className="rounded-3xl border border-slate-200/80 bg-white p-7 shadow-sm">
                <h2 className="text-2xl font-semibold text-slate-900">Die Herausforderung</h2>
                <p className="mt-3 text-slate-600">Viele Teilnehmende verstehen das Prüfungsdeutsch nicht sofort – obwohl sie die Inhalte in ihrer Sprache beherrschen.</p>
              </article>
              <article className="rounded-3xl border border-slate-200/80 bg-[linear-gradient(140deg,#0f2b54_0%,#0b3a75_70%)] p-7 text-white shadow-xl">
                <h2 className="text-2xl font-semibold">Unsere Lösung</h2>
                <p className="mt-3 text-slate-100">FachkundePilot übersetzt komplexe Themen didaktisch klar: verstehen, üben, auf Deutsch sicher antworten.</p>
              </article>
            </section>

            <section className="mt-12 rounded-3xl border border-white/70 bg-white/80 p-7 shadow-[0_16px_48px_rgba(15,23,42,0.08)] md:p-10">
              <h2 className="text-3xl font-semibold text-slate-900">Dein Lernpfad</h2>
              <div className="mt-8 grid gap-4 md:grid-cols-5">
                {["Recht", "Betrieb", "Rechnen", "Fragen", "Probeprüfung"].map((step, idx) => (
                  <article key={step} className="rounded-2xl border border-slate-200 bg-white p-4 text-center">
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Schritt {idx + 1}</p>
            ...
            <section className="mt-12 grid gap-6 md:grid-cols-2">
              <article className="rounded-3xl border border-white/70 bg-white/85 p-7 shadow-sm">
                <h2 className="text-2xl font-semibold text-slate-900">Mehrsprachige Unterstützung</h2>
              ...
              <article className="rounded-3xl border border-[var(--accent)]/40 bg-[var(--accent)]/10 p-7 shadow-sm">
                <h2 className="text-2xl font-semibold text-slate-900">Bereit für den nächsten Schritt?</h2>
                <p className="mt-3 text-slate-700">Starte mit einem klaren Plan und übe bis zur sicheren Prüfungsteilnahme.</p>
                <div className="mt-5">
                  <Link href={buildLocalizedPath(lang, "register")} className="inline-flex rounded-xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white">Jetzt kostenlos starten</Link>
                </div>
              </article>
            </section>
          </>
